  ...
  <div class="row">
    <div class="row__container container-horizontal padding-horizontal py-2">
      <div class="row__item image">
        <img :src="`${character.image}`" :alt="character.name" :class="{ greys: character.status.toLowerCase() === 'dead' }" />
        <RibbonIcon v-if="character.status.toLowerCase() === 'dead'" class="ribbon" />
      </div>
      <div class="row__item id">
        {{ character.id }}
      </div>
      ...
      <div class="row__item favorites">
        <button class="star" :class="{ favorite: character.isFavorite }" @click="toggleFavorite(character.id)">
          <StarIcon />
        </button>
      </div>
    </div>
